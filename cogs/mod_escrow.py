...
from .utils.db import (DecimalInvalidAmountError, DecimalPrecisionError,
                       EscrowAction, EscrowActioner, EscrowEvent,
...
class Escrow(commands.Cog):
    ...
    async def do_address_flow(self, msg, sender, receiver, currency, *, amount=None):
        ...
        if not receiver_address:
            try:
                query = await receiver.send(f"Someone wants to send you {f'{amount} ' if amount is not None else ''}{currency.value}, but you don't have an address on file for that currency...\nSend me your address now. (timeout in 60s)")
    ...
    async def escrow_send(self, ctx, recipient: MaybeRemoteMember, amount: Decimal, currency: CurrencyType, *, note: str = None):
        ...
        if maybe_transaction is not None:
            ...
            try:
                ...
                try:
                    sender_addr, receiver_addr = await self.do_address_flow(ctx.message, sender, recipient, currency, amount=verified_amount)
                    ...
                    await ctx.send(f"Sending {verified_amount} {currency.name} to {recipient.name}\n({sender_addr} -> {receiver_addr})\n> {note}\nPayment ID: {payment_id}")
        ...
        if maybe_transaction is None:
            ...
            if maybe_transaction.status == EscrowStatus.Received:
                ...
                did_report = await self.bot.db.create_payment_event(
                    ...
                    EscrowAction.Aborted,
                    EscrowActioner.Sender,
                    sender.id,
                    message=reason
                )
                ...
                if not (did_report and did_update):
                    log.critical(f"Could not write payment event for ({maybe_transaction.id}, s={maybe_transaction.sender}, r={maybe_transaction.receiver}")
                    raise RuntimeError("database write failed")

                await ctx.send(f"Aborted your pending transaction (ID: {maybe_transaction.id}) with {recipient.name}\n{f'> {reason}' if reason else ''}")
